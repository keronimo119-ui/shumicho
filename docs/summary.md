...
## 4. 現在の状態(2026-06-13時点)

- アプリ名を**「趣味帳Fav」**に改名済み(UI app-title + Android strings.xml)。ホーム=カレンダー。
- 6/11以降、**機能を大幅追加**(詳細は「7.経緯ダイジェスト」最新分を必読)。主なもの:
  - 観覧モード(図鑑/軌跡/俳句/番付を1点ずつめくる)+ギミック(餅・剣・波紋)。**餅=画像を局所変形して触れるぷにぷに**が看板機能。
  - 連続記録バッジ/イベント登録(記念日カウントダウン・カレンダー連動)/今日の季語の毎朝通知(宗教語は除外)。
  - 作品の画像シェア(ネイティブ共有)/おまかせスライドショー/写真込み丸ごとバックアップ。
  - ぷにぷに開封演出・コンプ祝福。手帳プレビュー強化。画像は**2本指ピンチでズーム**(スライダー廃止)。
- 全変更を実機(XQ-CC44, ワイヤレスadb)で検証しGitHub mainにpush済み(最新コミットまで)。
- **署名済みAABは未再ビルド**(v1.0.0/versionCode1の古いものだけ)。大量更新後なので**提出前に必ず再ビルド**。
- **AdMobは全部テストID**のまま(本番ID化は未着手)。Play Console未登録。
- プライバシーポリシー公開済み: https://keronimo119-ui.github.io/shumicho-privacy.html
- ⚠️新規プラグイン導入済み(cap sync済): `@capacitor/local-notifications@8` `@capacitor/share@8` `@capacitor/filesystem@8`。
  別PCでは `npm install` → `npx cap sync android` を必ず実行。
...
### 実装済み(2026-06-13・元アイデアから昇格)
- ✅ 画像シェア(観覧モードの📤) / ✅ 連続記録バッジ / ✅ 記念日カウントダウン(=イベント登録)

### 未実装アイデア(採否待ち)
- 図鑑「未入手だけ表示」フィルタ / 図鑑表紙写真 / 図鑑写真を軌跡のステッカーに呼び出す
- 番付の2色カラー連動 / 月場所アーカイブ(番付史)
- 年間句集書き出し / 俳句の連続記録バッジ(全体の連続記録はカレンダー基準で実装済み)
